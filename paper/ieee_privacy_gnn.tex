...
\begin{IEEEkeywords}
graph neural networks, prediction APIs, Big Data systems, selective release, calibration, membership inference
\end{IEEEkeywords}
...
None jointly provide a hop-expanded release set, constructor$\times$protector policy grid, Mass/Frac dials, matched-budget calibration, and release complexity under standard membership audits.
HARP targets that systems gap.
...
\begin{enumerate}
  ...
  \item \textbf{Systems headline.} Versus strong LBP: $+12.5$ Acc on Cora at Frac${=}0.40$; versus equal-mass: $60\%$ clean majority and ${\sim}5{\times}$ lower ECE; quantified cache-hit simulation for ExactFrac; clean-slice fidelity across datasets.
...
Our contribution is a \textbf{selective release framework} for GNN score APIs with Mass/Frac systems metrics.
Uniform output noise (LBP, MemGuard) and train-time defenses (GTD, MaskArmor) are baselines.
...
In short: LBP/MemGuard spend on every node; GTD/MaskArmor harden training without release Mass/Frac; GAP answers formal $\varepsilon$; HARP answers constrained exact-fraction release under audits.

\paragraph{Systems lens (why Big Data).}
Training systems optimize memory and throughput.
Score-API release is an adjacent surface: every prediction is a data product with a utility and quality cost.
HARP logs Frac and Mass the way operators already log batch size.
We compare against the published strong LBP recipe ($b{=}0.3$) and against equal-mass weak noise so reviewers see what the same budget buys when spent uniformly.
...
A synthetic mixer confirms the ordering: unprotected and seed-only leaf AUROCs stay near $1.0$, while hop-$1$ protection collapses the leaf attack (Fig.~\ref{fig:hop-cache}, left).
\end{proof}
...
Then setup (LTE + seeds + $k$-hop expand) runs in $O(m + n\log n)$ time and $O(n+m)$ space; release runs in $O(nC)$---matching uniform LBP's tier, with less noise-sampling work whenever $\mathrm{Frac}<1$.
\end{proposition}
\begin{proof}[Proof sketch]
...
Independent Lap$(\sigma)$ on every coordinate of $v\in\mathcal{P}_k$ yields $\varepsilon_{\mathrm{loc}}= \Delta/\sigma$ local DP for that node's release; nodes in $V\setminus\mathcal{P}_k$ have $\varepsilon{=}\infty$.
Consequently any policy with ExactFrac$>0$ has infinite \emph{global} $\varepsilon$---ExactFrac$>0$ and global DP are incompatible by construction.
\end{proposition}
...
This is the constrained-release tradeoff stated formally: HARP provides a finite local guarantee where it spends noise, and an explicit non-guarantee where it keeps scores exact.
...
It is an empirical selective release framework for the constrained problem: spend a protector on a hop-consistent minority, keep the rest exact, and audit with membership attacks.
...
\label{sec:discovery}
Matched-budget undefended LiRA (Table~\ref{tab:audit}) concentrates leakage in small/mid heterophilic cells; several mid/large graphs sit near chance under $n_{\mathrm{shadows}}{=}4$.
That audit motivates selective spend: do not pay strong uniform noise where LiRA${\approx}0.5$; deploy HARP where LiRA$\gg 0.5$ \emph{or} where veracity (exact/calibrated scores) is required under a strong-$\sigma$ policy.

\paragraph{Three deployment regimes.}
Heterophilic graphs (Chameleon/Actor) show auditable LiRA and LTE helps at fixed Frac; homophilic Cora/Citeseer emphasize veracity at matched Mass; large/audit-null graphs (PubMed/Photo/ogbn) often warrant Frac${=}0$ unless policy forces strong $\sigma$ (ogbn Acc recovery $+0.420$).
...
HARP yields ECE $0.033$ with Frac${=}0.40$---nearly $5{\times}$ better calibration---and a $60\%$ bit-exact clean majority.
Equal-mass cannot produce exact scores.
...
\caption{Left: hop necessity (Prop.~\ref{thm:hop-necessary})---seed-only leaves leaf AUROC at $1.0$; hop-$1$ collapses it. Right: score-cache hits under re-query traffic (Table~\ref{tab:cache}).}
\label{fig:hop-cache}
\end{figure}
...
Manual inspection of those $12$ failures shows a recurring pattern: Lap$(\sigma{=}0.3)$ plus renormalization can \emph{increase} true-class confidence on some draws (e.g., node $177$: $0.78\rightarrow 1.00$; node $381$: $0.34\rightarrow 0.81$), concentrating mass on the true class by chance.
Peaky clean posteriors (conf$>0.9$) are also hard to flatten.
The takeaway for operators: HARP reliably softens the mid-confidence mass that drives population LiRA, but a small overconfident/unlucky tail remains---raise Frac or $\sigma$ on that slice if canaries demand it, rather than expecting uniform-LBP-level erasure at ExactFrac${=}0.60$.

\paragraph{Adaptive LTE-aware queries.}
\label{sec:adaptive}
Table~\ref{tab:adaptive}: restricting queries to unprotected nodes, protected-set boundary cleans, or top-decile LTE seeds raises conf AUROC $0.513\rightarrow 0.638$--$0.651$ under locked HARP (3 seeds), while subset LiRA stays near population (${\approx}0.53$--$0.54$).
Population LiRA understates conf-MIA on the clean majority---log unprot./boundary canaries and raise Frac or $B$ if they drift.
Raising Frac to $0.60$ collapses the top-decile LTE canary ($0.651\rightarrow 0.505$) while residual clean-slice conf AUROC remains elevated (${\approx}0.63$)---pair Frac escalation with session budget $B{=}1$.
...
Table~\ref{tab:audit}(b) reports the honest shadow-count story on Cora for \emph{none / LBP / HARP}.
Absolute LiRA rises with $n_{\mathrm{shadows}}$ for every defense (Gaussian LiRA sharpens~\cite{carlini2022membership}); Acc is invariant.
\textbf{HARP does not beat strong LBP on LiRA at large shadow counts}---at $n_{\mathrm{sh}}{=}64$ on Cora, LBP LiRA $0.561$ vs.\ HARP $0.677$---expected when ExactFrac$=0$ is allowed.
What HARP keeps is (i)~strictly lower LiRA than undefended and (ii)~large Acc recovery vs.\ strong LBP ($\Delta$Acc $+0.12$, invariant in $n_{\mathrm{sh}}$).
...
Table~\ref{tab:canary-ogbn}(b): on ogbn-arxiv (${\sim}169$k; NeighborLoader; gate off, warmup $3$)~\cite{hu2020ogb}, strong LBP collapses Acc to $0.179$ $[0.175,0.184]$ while HARP recovers $0.599$ $[0.597,0.601]$ (paired $\Delta$Acc $+0.420$ $[0.414,0.425]$; 3 seeds) at Frac${=}0.40$.
Defense-aware LiRA stays near chance at both $n_{\mathrm{sh}}{=}2$ and $n_{\mathrm{sh}}{=}4$ (Table~\ref{tab:canary-ogbn}(b))---a volume Acc-recovery result under an audit-null regime, not a claim that ogbn needs Frac${>}0$.
On ogbn-products (${\sim}2.4$M nodes)~\cite{hu2020ogb}, Table~\ref{tab:products} reports the systems probe on a $16$\,GB host plus a defense-aware LiRA audit on a BFS-induced $15$k-node subgraph ($829$k edges; $n_{\mathrm{sh}}{=}2$).
HARP recovers Acc $0.343\rightarrow 0.686$ vs.\ strong LBP while LiRA stays near chance---the same Acc-recovery pattern as ogbn-arxiv, now with an honest large-graph audit.
...
The Big Data boundary: release accounting and loader serving scale to millions of nodes; full-graph shadow audits need more RAM or distributed training.
...
Cora LTE$+$hop setup ${\approx}41$\,ms; HARP release ${\approx}0.51$\,ms vs.\ LBP ${\approx}0.71$\,ms; products serving ${\approx}3.3{\times}10^3$ QPS at ${\approx}7.4$\,GB RSS.
...
Code (\texttt{defenses/harp.py}, \texttt{defenses/memguard.py}, \texttt{run\_harp\_rejection\_fix.py}), locked configs, and frozen CSVs accompany the submission artifact at \url{https://github.com/krishharma/privacy-gnn}.
