...
\title{HARP: A Selective Release Framework\\
...
\begin{abstract}
GNN prediction APIs release softmax scores used for ranking, routing, and analytics on graphs from thousands to millions of nodes.
At this scale the API is a continuous data product: operators need high accuracy, a majority of \emph{bit-exact} responses for caching and compliance (Veracity), and membership-audit compliance---while clients may re-query randomized answers (Velocity).
Uniform Laplace posterior perturbation (LBP), the standard release defense, cannot meet that joint goal: any $b{>}0$ perturbs \emph{every} response (ExactFrac${=}0$), and strong LBP collapses accuracy (e.g., $0.666\rightarrow 0.179$ on ogbn-arxiv).
We propose \textbf{HARP}, a hop-aware selective release \emph{framework} for GNN score APIs, together with a topology risk constructor (LTE) that ranks seeds and expands them by $k$ hops before applying a pluggable protector only on the protected set.
Frac, Mass, and session budget $B$ are first-class operator dials---release-side analogues of memory and batch budgets in training systems.
Across six graphs and OGB scale-ups (${\sim}169$k--$2.4$M nodes), HARP recovers up to $+42$ Acc points versus published strong LBP, cuts release Mass by $60\%$, enables a $60\%$ clean majority that uniform noise cannot provide, and raises score-cache hit rates from $0\%$ (fresh LBP) to $54\%$ from ExactFrac alone.
HARP is an audit-driven release policy---not a differential-privacy replacement.
\end{abstract}

\begin{IEEEkeywords}
graph neural networks, prediction APIs, Big Data systems, selective release, data quality, API serving
\end{IEEEkeywords}

\section{Introduction}

\subsection{Motivation}
GNN prediction APIs return softmax score vectors for ranking, triage, and analytics~\cite{kipf2017gcn,hamilton2017graphsage}.
At Big Data scale the API is not a one-shot model export---it is a high-volume stream of released probabilities with three operator constraints:
\textbf{Volume} (graphs from $10^3$ to $10^6+$ nodes),
\textbf{Velocity} (clients may re-query randomized answers),
and \textbf{Veracity} (pipelines need bit-exact, well-calibrated scores for caching, A/B tests, and compliance).
Those scores also leak training membership via MIAs~\cite{shokri2017membership,salem2019mlleaks,carlini2022membership,olatunji2021mia}.

The usual release defense is uniform Laplace posterior perturbation (LBP)~\cite{olatunji2021mia}.
It creates a hard systems failure under Veracity: any $b{>}0$ yields ExactFrac${=}0$, so score caches miss on every fresh draw, and strong LBP ($b{=}0.3$) further collapses Acc (ogbn-arxiv: $0.666\rightarrow 0.179$).
Weaker uniform noise keeps Acc but still perturbs every node.
Neither option provides a \emph{clean majority} of exact scores---the data-quality analogue of fitting a large graph into limited GPU memory.

\paragraph{Systems problem.}
Serve GNN scores so as to maximize Acc subject to a membership audit \emph{and} ExactFrac$\ge c{>}0$.
Uniform LBP is infeasible for $c{>}0$ by construction (Remark~\ref{rem:uniform-infeasible}).

\paragraph{Framework + algorithm.}
We propose \textbf{HARP}, a selective release \emph{framework}, with a hop-aware risk constructor (\textbf{LTE}) that ranks nodes from topology, selects seeds to hit a target Frac, and expands them by $k$ hops so message passing cannot reintroduce cues through clean neighbors.
A pluggable protector (strong Laplace or masking) runs only on $\mathcal{P}_k$; Frac/Mass/\(B\) are logged like training memory and batch budgets.
Under locked Frac${=}0.40$, about $60\%$ of responses stay bit-exact---impossible for any uniform $b{>}0$.

\paragraph{Headline quantitative wins.}
Versus published strong LBP: up to $+42$ Acc points on ogbn-arxiv and $+12.5$ on Cora; $60\%$ lower Mass; ExactFrac${=}0.60$; score-cache hits $0\%\rightarrow 54\%$ from ExactFrac alone (fresh LBP vs.\ HARP).
These are the systems answers to ``why accept this?''---not a claim of lower LiRA than uniform noise when ExactFrac${=}0$.
...
None treat ExactFrac$\ge c{>}0$ as a hard systems constraint on a hop-coupled GNN API with operator dials analogous to memory/batch budgets.
HARP targets that gap: a release \emph{framework} that reuses known protectors inside a new allocation problem.

\paragraph{Running example (Cora).}
Strong LBP cuts Acc ${\approx}0.88\rightarrow 0.71$; weak uniform noise preserves Acc but ExactFrac${=}0$ (ECE ${\approx}0.16$).
HARP at matched Mass keeps ExactFrac${=}0.60$, Acc ${\approx}0.83$, ECE ${\approx}0.033$---usable accuracy plus reproducible scores for most queries.

\paragraph{Non-goal.}
HARP is not a stronger Acc--LiRA defense than equal-mass LBP when ExactFrac${=}0$ is allowed.
The claim is ExactFrac feasibility, Acc recovery vs.\ strong LBP, and operator telemetry under that constraint.

\subsection{Contributions and paper map}
\begin{enumerate}
  \item \textbf{Systems headline.} Versus strong LBP: up to $+42$ Acc on ogbn-arxiv and $+12.5$ on Cora; $60\%$ lower Mass; ExactFrac${=}0.60$; cache hits $0\%\rightarrow 54\%$ from ExactFrac alone; ${\sim}5{\times}$ lower ECE \emph{on Cora} at matched Mass.
  \item \textbf{HARP framework + LTE constructor.} Constructor$\times$protector$\times$Mass/Frac$\times$session budget $B$, with hop-consistent protected sets and $O(|E|+n\log n)$ setup (Prop.~\ref{thm:harp-complexity}).
  \item \textbf{Constrained score release.} Formalize Acc s.t.\ LiRA$\le\tau$ and ExactFrac$\ge c$; Uniform LBP infeasible for $c{>}0$ (Remark~\ref{rem:uniform-infeasible}); hop-consistency (Prop.~\ref{thm:hop-necessary}); selective local-$\varepsilon$ accounting (Prop.~\ref{thm:local-eps}).
  \item \textbf{Scale audits.} Defense-aware LiRA on ogbn-arxiv ($n_{\mathrm{sh}}{\in}\{2,4\}$) and a products BFS subsample ($15$k of $2.4$M)~\cite{hu2020ogb}, plus GAT/inductive/adaptive checks.
\end{enumerate}
...
Uniform posterior noise wastes budget on low-risk nodes and cannot meet ExactFrac$\ge c{>}0$---the release-side analogue of loading an entire graph when GPU memory is scarce.
Node-local risk weighting is still incomplete for GNNs: if $v$ is protected but neighbor $u$ is released clean, aggregation can reintroduce membership signal into $u$'s score.
HARP is therefore a two-tier serving framework (Fig.~\ref{fig:harp}):
\begin{enumerate}
  \item \textbf{LTE constructor (algorithm):} rank nodes from topology (or audit-guided vulnerability / random), pick seeds to hit target Frac, expand $\mathcal{P}_k=\mathrm{Ball}_k(\mathcal{R})$.
  \item \textbf{Protector (pluggable):} strong Laplace on $\mathcal{P}_k$, or selective masking (exact on clean; one-hot top-1 on $\mathcal{P}_k$).
  \item \textbf{Operator dials:} Frac, Mass, session budget $B$; optional risk-weighted $\phi$-alignment at train time.
\end{enumerate}
Locked HARP uses LTE + Laplace at Frac${=}0.40$; audit seeds and masking are framework ablations.
LTE plays the role SPRING plays in streaming GNN training: a purpose-built allocator that makes the framework practical under a hard resource/quality constraint.
...
\subsection{Overview}
\paragraph{Why accept this paper (systems headline).}
Four numbers carry the Big Data claim:
(i)~ExactFrac${=}0.60$---infeasible for any uniform $b{>}0$;
(ii)~up to $+42$ Acc vs.\ strong LBP on ogbn-arxiv ($0.179\rightarrow 0.599$) and $+12.5$ on Cora;
(iii)~$60\%$ lower Mass than strong LBP at the same per-node $\sigma$;
(iv)~score-cache hits $0\%\rightarrow 54\%$ under fresh re-query traffic from ExactFrac alone (Table~\ref{tab:cache}).
LiRA is an audit dial that decides \emph{whether} Frac${>}0$, not the primary objective.

We then answer operator questions:
\textbf{(Q1)} Acc and score quality at matched Mass?
\textbf{(Q2)} Acc recovery vs.\ strong LBP?
\textbf{(Q3)} Frac, multi-query, and session budgets?
\textbf{(Q4)} Architectures, inductive splits, and OGB scale?
Equal-mass LBP remains the honest Acc--membership baseline when ExactFrac${=}0$ is allowed.

\subsection{How to read the trade space}
\textbf{Acc} $=$ task utility; \textbf{LiRA} $=$ membership audit; \textbf{ECE / ExactFrac} $=$ Veracity for downstream consumers.
Equal-mass LBP and MemGuard can win Acc--LiRA while ExactFrac${=}0$.
HARP's headline is Acc recovery plus ExactFrac$\ge c$ (Remark~\ref{rem:uniform-infeasible})---pick the constraint first, then read Tables~\ref{tab:fair}--\ref{tab:eqmass}.
...
Score HARP as Big Data \emph{infrastructure for score APIs}: a named framework (HARP) plus allocator (LTE), hard Veracity SLA (ExactFrac$\ge c$), Mass/Frac/\(B\) telemetry, and Acc/cache wins vs.\ strong LBP.
Do not score it as PETS/CCS privacy theory: no new $(\varepsilon,\delta)$ mechanism and no Acc--LiRA dominance claim under ExactFrac$=0$ (those are GAP / uniform noise).

\subsection{Impact for Big Data graph APIs}
Hop-consistent release is the serving-side analogue of topology-aware partitioners; Mass/Frac are the release-side analogues of GPU memory and batch size.
ExactFrac turns score caches from $0\%$ hits under fresh LBP into a majority hit rate (Table~\ref{tab:cache}); Acc recovery vs.\ $b{=}0.3$ restores usable pipelines on heterophilic and OGB graphs; audit-null regimes argue against blanket noise.
...
We proposed HARP, a selective release \emph{framework} for GNN prediction APIs under a hard Veracity constraint: ExactFrac$\ge c{>}0$.
Like memory-efficient training systems that refuse to load the full graph, HARP refuses to noise every response: LTE allocates a hop-consistent minority, a pluggable protector spends Mass only there, and Frac/\(B\) are operator dials.
Headline wins versus strong LBP---up to $+42$ Acc points, $60\%$ lower Mass, ExactFrac${=}0.60$, and $0\%\rightarrow 54\%$ score-cache hits---answer the systems ``why accept?'' question.
Membership audits guide \emph{when} to spend; use training-time DP when a legal global $\varepsilon$ is required.
