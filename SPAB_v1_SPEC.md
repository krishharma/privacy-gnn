...
- Volume (ogbn): 3 seeds; LiRA `n_shadows=2` in main grid; **`n_shadows=4` credibility CSV** for none+SAMI; **Tier A `n_shadows=16`** undefended GraphSAGE (`ogbn_lira_n16_3seed.csv`, LiRA≈0.510).
- Large+heterophilic peer: **arxiv-year** (LINKX year labels on ogbn-arxiv graph; `arxiv_year_lira_n4_3seed.csv`, h≈0.218, Acc≈0.395, LiRA≈0.501 at `n_shadows=4`).
